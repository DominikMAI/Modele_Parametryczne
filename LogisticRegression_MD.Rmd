  ...
  for (j in cols[1:length(cols)]){
    ...
    if ((i != 'CLAIM_FLAG')&(j != 'CLAIM_FLAG')){
      if (i != j){
        ...
        m <- glm(as.formula(paste('CLAIM_FLAG', "~INCOME+", i,":",j)), 
                 data = dane_uczacy, 
                 family = binomial)
        
        vif_t <- vif(m)
        wspl <- as.numeric(vif_t[paste(i,':',j,sep='')])[1]
        print(paste(wspl))
        if (wspl <= 1.05){
          w_test <- waldtest(m)
          if (w_test$`Pr(>F)`[2] < 1){
...
**błąd** - iloraz sumy niepoprawnie sklasyfikowanych próbek przez sumę wszystkich**
**precyzja** - 
