...
```
Etap modelowania oszacowania szansy wystąpienia kolizji drogowej i wystąpienia o odszkodowanie przez klienta w oparciu o dane dotyczące: <br/>
...
Kolejne cechy zostały dobierane w oparciu o 
...
- Studenci i osoby nisko wykwalifikowane rzadziej biorą udział w kolizjach drogowych.

- Liczba dzieci wpływa na ryzyko udziału w kolizji drogowej przez rodziców.
...
```
**Wniosek**
Ze względu na to, że zmienna REVOKED obniża wartość kryterium informacyjnego, decydujemy o włączaniu jej do modelu. Fakt utraty prawa jazdy zwiększa szanse na 
udział w kolizji w pszyszłości.
...
```
**Wniosek**
Wykorzystanie zmiennej charakteryzującej jednostkę osadniczą pozwoliło uzyskać model, który uzyskał wartość kryterium Akaike o 591.8558 punktów niższą względem najlepszego dotychczas modelu.Parametry strukuralne są istotne. 
Zmienna zostaje włączona do modelu.


### Model 14 -  typ zatrudnienia
Zakładamy, że osoby zatrudnieni w zawodach wymagających wysokich kompetencji rzadziej biorą udział w kolizjach drogowych.
```{r}
...
```
**Wniosek: **Zmienna nie poprawia znacząco naszego modelu (jedynie o 3.51 punkta AIC)
...
```
**Wniosek**
Zmienna pogarsza nasz model o 535.83 punktów kryterium informacyjnego AIC.
...
summary(m16)
```
**Wniosek:** Liczba dzieci nie a wpływu na szanse wystąpienia kolizji, wnioskujemy, że sam fakt zmniejsza szansę udziału w kolizji.

```{r}
summary(m13)$aic - summary(m16)$aic
```
**Wniosek**
Zmienna liczby dzieci znacznie zwiększa wartość kryterium informacyjnego.
...
**dokładność -** - iloraz sumy poprawnie sklasyfikowanych próbek przez sum wszystkich próbek. <br/>
**błąd -** iloraz sumy niepoprawnie sklasyfikowanych próbek przez sumę wszystkich </br>
**czułość - **zdolność modelu do wychwytywania przypadków pozytywnych<br/>
**swoistość - ** zdolność modelu do wychwytywania przypadków negatywnych<br/>
**
```{r}
