...
Zmienna MSTATUS nie zostaje włączona do modelu
...
```
**Wniosek**
Zmienna BlueBook pogarsza nasz model podwyyższajac wartość kryterium informacyjnego Akaike.
...
```{r}
m8 <- glm(CLAIM_FLAG~INCOME+PARENT1+AGE_tr+HOME_VAL_D+TIF, data = dane_uczacy, family = binomial)
summary(m8)
```
**Wniosek**
Zmienna TIF pogarsza nasz model podwyyższajac wartość kryterium informacyjnego Akaike.
Zmienna TIF nie zostaje włączona do modelu, na podstawie dedukcji.


### Model 10 - Liczba lat jako klient

```{r}
m8 <- glm(CLAIM_FLAG~INCOME+PARENT1+AGE_tr+HOME_VAL_D+CAR_TYPE, data = dane_uczacy, family = binomial)
summary(m8)
```
**Wniosek**
NO i nie wiem
