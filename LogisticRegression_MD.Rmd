...
Zmienna **CAR_USE** zostaje włączona do modelu, jej właczenie pozwala na obniżenie kryterium informacyjnego o 152.62 jednostek.

### Model 8 - Cena samochodu
```{r}
m8 <- glm(CLAIM_FLAG~INCOME+PARENT1+AGE_tr+HOME_VAL_D+CAR_USE+BLUEBOOK, data = dane_uczacy, family = binomial)
summary(m8)
```
```{r}
...
**Wniosek**
Zmienna BlueBook poprawia dopasowanie naszego modelu, obniżając wartość kryterium informacyjnego Akaike 28.26 jednostek.
Zmienna bluebook zostaje włączona do modelu.


### Model 9 - liczba lat jako klient
```{r}
m9 <- glm(CLAIM_FLAG~INCOME+PARENT1+AGE_tr+HOME_VAL_D+CAR_USE+BLUEBOOK+TIF, data = dane_uczacy, family = binomial)
summary(m9)
```
...
Zmienna obniża wartość kryterium informacyjnego Akaike o 48.14.
